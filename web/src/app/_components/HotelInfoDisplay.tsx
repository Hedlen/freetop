...
export function HotelInfoDisplay({ content, className }: HotelInfoDisplayProps) {
  ...
  return (
    <div className={cn("space-y-6", className)}>
      {/* 酒店卡片 */}
      <div className="rounded-xl border border-gray-200/60 bg-gradient-to-r from-gray-50/80 to-blue-50/60 p-6 shadow-sm backdrop-blur-sm">
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-4">
            <div className="rounded-full bg-blue-100/80 p-3">
              <svg className="h-8 w-8 text-blue-600/80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex-1">
              ...
              {hotelInfo.location && (
                <div className="mt-1 flex items-center text-gray-600">
                  ...
                  <span className="text-sm">{hotelInfo.location}</span>
                </div>
              )}
              {hotelInfo.rating && (
          ...
          {hotelInfo.price && (
            <div className="text-right">
              <p className="text-3xl font-bold text-blue-600/90">¥{hotelInfo.price}</p>
              <p className="text-sm text-gray-500">每晚起</p>
            </div>
          )}
        </div>
        ...
        {hotelInfo.amenities && hotelInfo.amenities.length > 0 && (
          <div className="mt-4">
            <h4 className="mb-2 font-medium text-gray-900">酒店设施</h4>
            <div className="flex flex-wrap gap-2">
              {hotelInfo.amenities.map((amenity, index) => (
                <span
                  key={index}
                  className="rounded-full bg-white/80 px-3 py-1 text-xs font-medium text-gray-700 shadow-sm border border-gray-200/50"
                >
                  {amenity}
                </span>
              ))}
